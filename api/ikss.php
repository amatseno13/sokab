...
try {
    ...
    if ($method === 'GET' && $action === 'list') {
        ...
        $sql = "SELECT
                    ...
                    p.iku_kode
                FROM ikss_master m
                LEFT JOIN ikss_links l
                       ON m.id = l.ikss_id AND l.triwulan = ? AND l.tahun = ?
                LEFT JOIN ck_iku_ikss p
                       ON p.ikss_id = m.id
                WHERE m.is_active = 1
                ORDER BY CAST(m.nomor AS UNSIGNED)";

        $stmt = $pdo->prepare($sql);
        $stmt->execute([$triwulan, $tahun]);
        $data = $stmt->fetchAll();

        // periode_id (ck_periode) untuk triwulan+tahun yang sama — dipakai tombol
        // "Generate Dokumen Sumber". Sama untuk "Tautan Bukti TL Sebelumnya", tapi
        // mundur satu triwulan (TW I mundur ke TW IV tahun sebelumnya) — dokumennya
        // struktur SAMA, cuma datanya diambil dari ck_entry_iku periode yang lebih tua.
        $tw_short = trim(preg_replace('/^TW\s*/i', '', $triwulan));
        $stmt2 = $pdo->prepare("SELECT id FROM ck_periode WHERE tahun = ? AND triwulan = ?");
        $stmt2->execute([$tahun, $tw_short]);
        $periode_id = $stmt2->fetchColumn();

        $urut = ['I', 'II', 'III', 'IV'];
        $idx  = array_search($tw_short, $urut, true);
        if ($idx === false) $idx = 0;
        $tw_sblm    = $urut[($idx + 3) % 4];        // mundur satu, TW I → TW IV
        $tahun_sblm = ($idx === 0) ? $tahun - 1 : $tahun;
        $stmt2->execute([$tahun_sblm, $tw_sblm]);
        $periode_id_sblm = $stmt2->fetchColumn();
        ...
        echo json_encode([
            ...
            'triwulan' => $triwulan,
            'tahun' => $tahun,
            'periode_id' => $periode_id ? (int)$periode_id : null,
            'periode_id_sblm' => $periode_id_sblm ? (int)$periode_id_sblm : null
        ]);
        exit();
    }
    ...
    if ($method === 'GET' && $action === 'detail') {
        ...
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$triwulan, $tahun, $ikss_id]);
        $data = $stmt->fetch();
    ...
    if ($method === 'POST' && $action === 'update_link') {
        ...
        $check = $pdo->prepare("SELECT id FROM ikss_links WHERE ikss_id = ? AND triwulan = ? AND tahun = ?");
        $check->execute([$ikss_id, $triwulan, $tahun]);
        ...
        if ($existing) {
            ...
            // Insert
            $sql = "INSERT INTO ikss_links (ikss_id, triwulan, tahun, link_dokumen_sumber, link_tindak_lanjut, updated_by) 
                    VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                $ikss_id,
