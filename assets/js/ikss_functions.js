...
let currentPeriodeId = null;      // ck_periode.id untuk triwulan+tahun aktif — dipakai tombol Generate Dokumen Sumber
let currentPeriodeIdSblm = null;  // ck_periode.id triwulan SEBELUMNYA — dipakai tombol Generate di kolom TL Sebelumnya
...
// 1b. TAHUN AKTIF
// ============================================
/** Ambil tahun dari dropdown ikssFilterTahun; jatuh ke tahun berjalan bila belum ada. */
function getIKSSTahun() {
    const el = document.getElementById('ikssFilterTahun');
    const th = el && el.value ? parseInt(el.value, 10) : NaN;
    return isNaN(th) ? new Date().getFullYear() : th;
}
...
function loadIKSSData(triwulan) {
    ...
    fetch(`api/ikss.php?action=list&triwulan=${encodeURIComponent(triwulan)}&tahun=${getIKSSTahun()}`)
        .then(res => res.json())
        .then(data => {
            console.log('📊 IKSS data received:', data);
            if (data.success) {
                currentIKSSData = data.data;
                currentPeriodeId = data.periode_id || null;
                currentPeriodeIdSblm = data.periode_id_sblm || null;
            ...
            <tr style="border-bottom: 1px solid #e2e8f0;">
                ...
                <td style="padding: 0.5rem; vertical-align: middle;">
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        ` : ''}
                        ${ikss.iku_kode && currentPeriodeId ? `
                            <button onclick="generateDokumenSumberIKSS(${currentPeriodeId}, '${ikss.iku_kode}', this)"
                                    title="Susun dokumen bukti dukung (narasi + foto) dari Entry Capaian Kinerja"
                                    style="padding: 0.35rem 0.75rem; background: #e67e22; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.75rem; white-space: nowrap; font-weight: 500;">
                                📄 Generate
                            </button>
                        ` : ''}
                    </div>
                ...
                <td style="padding: 0.5rem; vertical-align: middle;">
                    <div style="display: flex; align-items: center; gap: 0.5rem;">
                        ...
                        ` : ''}
                        ${ikss.iku_kode && currentPeriodeIdSblm ? `
                            <button onclick="generateDokumenSumberIKSS(${currentPeriodeIdSblm}, '${ikss.iku_kode}', this)"
                                    title="Susun dokumen bukti dukung dari data triwulan sebelumnya"
                                    style="padding: 0.35rem 0.75rem; background: #e67e22; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.75rem; white-space: nowrap; font-weight: 500;">
                                📄 Generate
                            </button>
                        ` : ''}
...
function simpanLinkDokumen() {
    ...
    formData.append('tahun', getIKSSTahun());
...
function editIKSSMaster(id) {
    ...
    fetch(`api/ikss.php?action=detail&ikss_id=${id}&triwulan=${encodeURIComponent(currentTriwulan || 'TW I')}&tahun=${getIKSSTahun()}`)
        .then(res => res.json())
        .then(result => {
            if (result.success) {
...
// ============================================
// 7b. GENERATE DOKUMEN SUMBER (narasi RO + foto bukti dari Entry Capaian Kinerja)
// ============================================
async function generateDokumenSumberIKSS(periodeId, ikuKode, btn) {
    const asli = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '⏳...';

    try {
        const res = await fetch(`api/notula_api.php?action=generate_dokumen_sumber&periode_id=${periodeId}&iku_kode=${encodeURIComponent(ikuKode)}`);
        ...
        const nama = (disp.match(/filename="(.+?)"/) || [])[1] || `Bukti_Dokumen_Sumber_${ikuKode}.docx`;
        ...
        showToast('Dokumen sumber berhasil dibuat' + extra, 'success');
    } catch (e) {
        showToast('Gagal generate dokumen sumber: ' + e.message, 'error');
    } finally {
        btn.disabled = false;
        btn.innerHTML = asli;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Dropdown tahun sebelumnya hanya tampil tanpa efek — sekarang memuat ulang data
    const selTahun = document.getElementById('ikssFilterTahun');
    if (selTahun) {
        selTahun.addEventListener('change', function () {
            console.log('📅 Tahun IKSS diganti:', this.value);
            loadIKSSData(currentTriwulan || 'TW I');
        });
    }
